...
反应奖励: ≤ 500ms +50%, ≤ 800ms +20%, ≤ 1200ms +4%, 否则无奖励
连击加成: combo ≥ 10 时每 hit +10% (最高 +100%)

最终得分 = 基础分 × (1 + 反应奖励 + 连击加成)
```

> 调整说明: 阈值设计为"快即奖励",>1200ms 视为正常反应,无额外奖励。Spec 初稿的 ≤1.0s/≤1.5s 阈值会让普通反应也有奖励,弱化了"快"的辨识度。
...
**不**采用通用"可爱卡通 + 粉蓝渐变"的 AI 默认配色。改走 **手绘故事书插画 + 中式印章美学** 方向:
- 灵感来自《中国童谣画本》《丰子恺漫画》、日式木刻版画
- 暖纸色背景,深墨色轮廓,3 个鲜明主色 + 1 个金黄强调
- 角色手绘感 (略带抖动的不规则线条),而非干净的几何
- 地鼠身上字符呈"印章"样式 (圆形篆刻红章)

### 8.1 色板

**通用 AI 默认**:奶油 + 衬线 + 赤土。**避免**。

**项目调色板**:

| 角色 | 名称 | Hex | 用途 |
|------|------|------|------|
| 纸 | paper | `#F5EBD7` | 主背景,像旧绘本纸 |
| 纸暗 | paper-shadow | `#E8DAB8` | 卡片底,带颗粒纹理 |
| 墨 | ink | `#2C1810` | 文本、轮廓(非纯黑,带温度) |
| 墨淡 | ink-muted | `#7A5C3F` | 副文本 |
| 朱砂 | vermilion | `#C44536` | 印章、强调、miss 反馈 |
| 赭石 | ochre | `#D4673A` | 猴子、地鼠、数字徽章 |
| 苔绿 | moss | `#5A8068` | 草地、success 反馈 |
| 雾蓝 | haze | `#7BA7BC` | 远山、天空高部 |
| 蜜金 | honey | `#DAA520` | 成就、解锁强调色 |

天空渐变: `haze → #F2C9A7` (蜜桃,不是粉色)

**配色规则**:
- 同一画面最多出现 4 个色 (含背景)
- 主色面积占 ~60%,强调色 (朱砂/蜜金) < 10%
- 轮廓始终用 ink,不混用其他深色

### 8.2 字体

```css
--font-display: 'ZCOOL KuaiLe', 'Ma Shan Zheng', 'STKaiti', system-ui;  /* 中式手写体 */
--font-display-en: 'Fraunces', 'Georgia', serif;                       /* 英文标题,带书写感 */
--font-ui: 'Noto Sans SC', 'Nunito', system-ui, sans-serif;             /* UI 文本 */
--font-key: 'JetBrains Mono', 'Menlo', monospace;                       /* 等宽,字符清晰 */
```

**字体策略**:
- 中文标题用 **ZCOOL KuaiLe** (免费 Google Fonts,圆润手写感强)
- 英文标题用 **Fraunces** (可变字体,有书写风的 serif)
- UI 用 Noto Sans SC (中文清晰)
- 字符 (字母/数字) 用 JetBrains Mono (等宽,易识别)
...
```css
background-image: 
  radial-gradient(circle at 20% 30%, rgba(44,24,16,0.04) 1px, transparent 1px),
  radial-gradient(circle at 70% 60%, rgba(44,24,16,0.03) 1px, transparent 1px);
background-size: 8px 8px, 12px 12px;
```

**墨轮廓** (CSS 或 SVG): 角色外轮廓用 `stroke: var(--ink); stroke-width: 2.5; stroke-linejoin: round;` 而非纯填充。

**印章感**: 地鼠身上的字符用红章样式 (圆形白底 + 朱砂字),象征"按下字符 = 盖章"。

### 8.4 动画原则

| 类型 | 时长 | 缓动 | 用途 |
|------|------|------|------|
| 弹性进场 | 350ms | `cubic-bezier(0.34, 1.56, 0.64, 1)` | 地鼠弹出、按钮 |
| 墨迹渲染 | 800ms | `ease-out` | 首屏猴子绘制 |
| 弹跳反馈 | 450ms | overshoot | 击中 |
| 摇晃 | 500ms | `ease-in-out` | miss |
| 印章盖下 | 250ms | `cubic-bezier(0.6, 0.04, 0.2, 1)` | 字符盖在地鼠身上 |
...
- **小猴子 (yunhou)**:
  - 赭石色身体,圆润手绘感
  - 长尾巴,挥动时晃动
  - 持木槌,idle 时槌在肩,挥击时画弧 + 猴身压低
  - hit 后原地小跳 + 表情喜悦
  
- **地鼠**:
  - 圆润造型,黑豆眼,两颗门牙
  - 头顶红印章显示字符 (朱砂红字 + 圆形白底 + 墨色边)
  - hit 后印章盖下变实心 + 碎成"墨点"粒子

- **地洞**:
  - 椭圆土堆,赭石色,顶部有 1-2 根草叶 (随机方向)
  - 12 个均匀分布在 4×3 网格
...
主菜单避免 "4 个并列卡片" 通用模板。改用:
- 一只大手绘猴子占据主视觉 (顶部)
- 标题用 ZCOOL KuaiLe 手写体,带墨色阴影
- 4 个场景作为下方按钮 (不是卡片网格)
- 背景: 纸纹理 + 远景山墨色简笔画

**首页核心 hero 是猴子形象本身**,不是 "标题 + 卡片" 模板。
